...
class RobotArmMotor:
    """
    Klasse voor het aansturen van één specifieke motor van de robotarm of kop (pan/tilt).

    Hiermee kun je een motor naar een specifieke positie bewegen, of hem terugsturen naar zijn nulpunt ('home').
    """
    ...
    instance_ENUM: int
    '''Enum die aanduidt welk onderdeel deze motor aanstuurt.'''

    instance_name: str
    '''Volledige naam zoals "robot.left_arm.motor".'''

    maximum_position: int
    '''Maximale toegestane positie voor deze motor (standaard: 500).'''

    minimum_position: int
    '''Minimale toegestane positie voor deze motor (standaard: 0).'''

    def __init__(self, bridge_manager, parent_name, instance_ENUM, maximum_position=None, minimum_position=None) -> None:
        """
        Initialiseert een RobotArmMotor-object.

        Args:
            bridge_manager (object): Interface naar de robot.
            parent_name (str): Naam van het bovenliggende onderdeel.
            instance_ENUM (int): Enum die deze motor aanduidt.
            maximum_position (int, optional): Maximale positie (default: 500).
            minimum_position (int, optional): Minimale positie (default: 0).
        """
    ...
    def move(self, position) -> None:
        """
        Beweegt de motor naar de opgegeven positie, mits deze binnen de toegestane grenzen valt.

        Args:
            position (int): Doelpositie (tussen minimum en maximum).
        """
    ...
    def home(self) -> None:
        """
        Stuurt de motor terug naar de nulpunt-positie ('home').

        Welke opdracht wordt gestuurd, is afhankelijk van het type motor.
        """
        if self.instance_ENUM == SaraRobotPartNames.LEFT_ARM_MOTOR:
            self.bridge_manager.cmd_Generic(
